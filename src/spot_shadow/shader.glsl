...
void main() {
    ...
    out_position_shadow = deref(push.light_buffer).light_matrix * deref(push.object_info).model_matrix * vec4(deref(push.vertices[gl_VertexIndex]).position, 1.0);
...
void main() {
    ...
    color *= 0.1;
#if USE_PCF == 0
    f32 shadow = max(calculate_shadow(deref(push.light_buffer).shadow_image, deref(push.light_buffer).shadow_sampler, in_position_shadow / in_position_shadow.w, vec2(0.0, 0.0), push.bias), push.shadow_intensity);
#else
    ...
    f32 light_inner_cut_off = normalize(deref(push.light_buffer).inner_cut_off);
    f32 light_outer_cut_off = normalize(deref(push.light_buffer).outer_cut_off);
    ...
    out_color = f32vec4(color + color * f32vec3(1.0) * ((diffuse + exp(exponent)) * shadow) * attenuation * 64.0f * intensity, 1.0);
}
