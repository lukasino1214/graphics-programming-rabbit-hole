...
void main() {
    ...
    out_normal = normalize(f32mat3x3(deref(push.matrices).normal_matrix) * deref(push.vertices[gl_VertexIndex]).normal);
    gl_Position = deref(push.matrices).mvp * f32vec4(deref(push.vertices[gl_VertexIndex]).position, 1.0);
}
